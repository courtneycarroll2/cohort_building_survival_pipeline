...
qc_genomic <- function(genomic_data, cohort_model_specs) {
  ...
  if (!is.null(cohort_model_specs$qc$proportion_missing_rm_variable) & !is.null(genomic_data)) {
    ...
    keep_indices <- (colSums(is.na(genomic_data[,2:ncol(genomic_data)]))/nrow(genomic_data)) < pct
    keep_samples <- colnames(genomic_data[,2:ncol(genomic_data)][keep_indices])
    
    # qc output:
    stage <- paste0("rm_missing_genomic_gt_", pct, "_pct")
    n_in <- ncol(genomic_data)-1
